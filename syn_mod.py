...
from utils import plot, PARAMS_NAME, load_json, load_hparams, add_prefix, add_postfix, get_time, parallel_run, makedirs, \
    str2bool
...
class Synthesizer(object):
    ...
    def load(self, checkpoint_path, num_speakers=2, checkpoint_step=None, inference_prenet_dropout=True,
             model_name='tacotron'):
        ...
        with tf.variable_scope('model') as scope:
            ...
            self.model.initialize(inputs=inputs, input_lengths=input_lengths, num_speakers=self.num_speakers,
                                  speaker_id=speaker_id, is_training=False)
    ...
    def synthesize(self,
        ...
        if speaker_ids is not None:
            if type(speaker_ids) == dict:
                ...
                speaker_embed = [speaker_ids[speaker_id] * speaker_embed_table[speaker_id] for speaker_id in
                                 speaker_ids]
...
if __name__ == "__main__":
    ...
    audio = np.zeros(1)
    texts = ['은채야 안녕 삼촌이야', '이거 삼촌이 만든거야', '내일 여행 잘 다녀오렴', '감기걸리지 말고', '안녕 다음에 보자']
    for text in texts:
        audio = np.append(audio, synthesizer.synthesize(texts=[text], base_path=config.sample_path,
                                                        speaker_ids=[config.speaker_id],
                                                        attention_trim=True,
                                                        base_alignment_path=config.base_alignment_path,
                                                        isKorean=config.is_korean))
        audio = np.append(audio, np.zeros(5000))

    save_wav(audio, r'C:\Users\김규림\pythonProject\tacotron2\sample\hi.wav', 24000)
